...
class SimpleBlock(nn.Module):
    def __init__(self, indim, outdim, stride, dilated_rate):
        ...
        self.C1 = nn.Conv2d(indim, outdim, kernel_size=3, stride=self.stride, dilation=self.dilated_rate,
                            padding=self.dilated_rate, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, list_of_out_dims=[64,128,256,512], list_of_stride=[1,2,2,2],
    ...
    def forward(self, x):
        out = torch.mean(self.trunk(x), [2, 3])  # (B,64,56,56)
        return out
